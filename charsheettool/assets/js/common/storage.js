...
async function loadInfusions(fallback = []) {
  return safeJSONMulti(candidatesFor("infusions.json"), fallback);
}
...
async function getRaceFeatures(character) {
  // --- NEW: Prioritize traits defined directly on the character object ---
  if (Array.isArray(character.traits) && character.traits.length > 0) {
    // Ensure the traits have a source for display consistency
    return character.traits.map(t => ({ ...t, source: t.source || 'Racial' }));
  }
...
function readActionState(c) {
  try { return JSON.parse(localStorage.getItem(getActionStateKey(c)) || '{}'); } catch { return {}; }
}
...
window.getCurrentCharactaer    = window.getCurrentCharacter    || getCurrentCharacter;
...
function readSpellState(c) {
  try { return JSON.parse(localStorage.getItem(getSpellStateKey(c)) || '{}'); } catch { return {}; }
}
...
function readAllActiveInfusions() {
  return STORAGE.get(ALL_INFUSIONS_KEY, []);
}
